class __$version__::contrail_compute {
...
define compute-scripts {
    ...
    exec { "setup-${title}" :
        command => "/opt/contrail/bin/${title}.sh; echo setup-${title} >> /etc/contrail/contrail_compute_exec.out",
        require => File["/opt/contrail/bin/${title}.sh"],
        unless  => "grep -qx setup-${title} /etc/contrail/contrail_compute_exec.out",
        provider => shell,
        logoutput => "true"
    }
}
...
define contrail_compute_part_2 (
    ...
    exec { "exec-compute-qpid-rabbitmq-hostname" :
        command => "echo \"rabbit_host = $contrail_amqp_server_ip\" >> /etc/nova/nova.conf && echo exec-compute-qpid-rabbitmq-hostname >> /etc/contrail/contrail_compute_exec.out",
        unless  => ["grep -qx exec-compute-qpid-rabbitmq-hostname /etc/contrail/contrail_compute_exec.out",
    ...
    exec { "exec-compute-neutron-admin" :
        command => "echo \"neutron_admin_auth_url = http://$contrail_keystone_ip:5000/v2.0\" >> /etc/nova/nova.conf && echo exec-compute-neutron-admin >> /etc/contrail/contrail_compute_exec.out",
	...
	if ( $contrail_vm_ip != "" ) {
        ...
        # Ensure all config files with correct content are present.
        compute-template-scripts { [ "agent_param.tmpl",
                                    "rpm_agent.conf",
                                    "vnc_api_lib.ini",
	...
	$update_dev_net_cmd = "/bin/bash -c \"python /etc/contrail/contrail_setup_utils/update_dev_net_config_files.py --compute_ip $contrail_compute_ip --physical_interface \'$contrail_physical_interface\' --non_mgmt_ip \'$contrail_non_mgmt_ip\' --non_mgmt_gw \'$contrail_non_mgmt_gw\' --collector_ip $contrail_collector_ip --discovery_ip $contrail_config_ip --ncontrols $contrail_num_controls --mac $contrail_macaddr && echo update-dev-net-config >> /etc/contrail/contrail_compute_exec.out\""
        ...
        file { "/etc/contrail/contrail_setup_utils/provision_vrouter.py":
            ...
            owner => root,
            group => root,
            source => "puppet:///modules/$module_name/provision_vrouter.py"
        }
        exec { "add-vnc-config" :
        ...
        Package['contrail-openstack-vrouter'] -> File["/etc/libvirt/qemu.conf"] -> Compute-template-scripts["vrouter_nodemgr_param"] -> Compute-template-scripts["agent_param.tmpl"] ->  Compute-template-scripts["rpm_agent.conf"] -> File["/etc/contrail/contrail_setup_utils/update_dev_net_config_files.py"] -> Exec["update-dev-net-config"] -> Exec["update-compute-nova-conf-file1"] -> Exec["update-compute-nova-conf-file2"] ->  Compute-template-scripts["contrail-vrouter-agent.conf"] -> File["/etc/contrail/contrail_setup_utils/provision_vrouter.py"]->Compute-template-scripts["vnc_api_lib.ini"]-> Exec["add-vnc-config"] -> Compute-scripts["compute-server-setup"] -> File["/etc/contrail/interface_renamed"] -> Exec["reboot-server"]
    }
    else {
        file { "/etc/contrail/contrail_setup_utils/update_dev_net_config_files.py":
            ensure  => present,
        ...
        exec { "update-dev-net-config" :
            command => "/bin/bash \"python /etc/contrail/contrail_setup_utils/update_dev_net_config_files.py --compute_ip $contrail_compute_ip --physical_interface \'$contrail_physical_interface\' --non_mgmt_ip \'$contrail_non_mgmt_ip\' --non_mgmt_gw \'$contrail_non_mgmt_gw\' --collector_ip $contrail_collector_ip --discovery_ip $contrail_config_ip --ncontrols $contrail_num_controls --mac $contrail_macaddr && echo update-dev-net-config >> /etc/contrail/contrail_compute_exec.out\"",
            require => [ File["/etc/contrail/contrail_setup_utils/update_dev_net_config_files.py"] ],
            unless  => "grep -qx update-dev-net-config /etc/contrail/contrail_compute_exec.out",
            provider => shell,
            logoutput => 'true'
