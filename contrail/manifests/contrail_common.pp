...
define upgrade-kernel($contrail_kernel_version) {
    ...
    $image_extra = "linux-image-extra-${contrail_kernel_version}-generic"
...
#	$contrail_member_list = inline_template('<%= contrail_members.delete! "" %>')
	...
	if($contrail_members == "" ) {
	    ...
	    $exec_cmd = "python /usr/local/lib/python2.7/dist-packages/contrail_provisioning/common/interface_setup.py --device $contrail_device --ip $contrail_ip"
	} else {
	    $exec_cmd = "python /usr/local/lib/python2.7/dist-packages/contrail_provisioning/common/interface_setup.py --device $contrail_device --members $contrail_intf_member_list_for_shell --bond-opts \"$contrail_bond_opts\" --ip $contrail_ip"
	}
