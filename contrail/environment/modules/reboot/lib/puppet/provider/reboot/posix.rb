Puppet::Type.type(:reboot).provide :posix do
  ...
  def cancel_transaction
    Puppet.warning("###DEBUG cancel transaction")
    Puppet::Application.stop!
  end

  def reboot
    Puppet.warning("###DEBUG  in reboot")
    if @resource[:apply] != :finished
      Puppet.warning("###DEBUG  DO cancel transaction")
      cancel_transaction
    end
  ...
  def async_shutdown(shutdown_cmd)
    Puppet.warning("Adding #{shutdown_cmd} to ruby's at_exit handler")
