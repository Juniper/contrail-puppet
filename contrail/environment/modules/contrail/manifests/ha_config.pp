...
class contrail::ha_config (
    ...
    if($internal_vip != '' and $host_control_ip in $openstack_ip_list) {
        ...
        if ('config' in $host_roles and !('database' in $host_roles) and $config_manage_db == true) {
            $zookeeper_ip_list = $::contrail::params::config_ip_list
        } elsif ('database' in $host_roles and !('config' in $host_roles) and $config_manage_db == true) {
            $zookeeper_ip_list = $::contrail::params::config_ip_list
