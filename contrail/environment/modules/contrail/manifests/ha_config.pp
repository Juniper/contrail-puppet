...
#
define check-galera ($openstack_index,
...
define check-galera-prime ($openstack_index,
                     $os_master_ip,
                     $os_master_user,
                     $os_master_passwd,
            ) {
    ...
    if($openstack_index != 1 ) {
	file { "/opt/contrail/bin/check_galera_prime.py" :
	    ensure  => present,
	    mode => 0755,
	    group => root,
	    source => "puppet:///modules/$module_name/check_galera_prime.py"
	}
	->
	exec { "exec_check_galera" :
	    command => "python /opt/contrail/bin/check_galera_prime.py $os_master_ip $os_master_user $os_master_passwd && echo check_galera_prime >> /etc/contrail/contrail_ha_exec.out",
	    cwd => "/opt/contrail/bin/",
	    unless  => "grep -qx check_galera_prime /etc/contrail/contrail_ha_exec.out",
	    provider => shell,
	    require => [ File["/opt/contrail/bin/check_galera_prime.py"] ],
	    logoutput => 'true',
	}
    } 
...
define fix-wsrep ($openstack_index,
                    $openstack_mgmt_ip_list_shell, 
                    $openstack_ip_list_wsrep,
                   )
{
    ...
    if ($openstack_index == 1) {
        ...
        exec { "exec_check_wsrep" :
            command => "python /opt/contrail/bin/check-wsrep-status.py $openstack_mgmt_ip_list_shell  && echo check-wsrep >> /etc/contrail/contrail_ha_exec.out",  
...
class contrail::ha_config (
	...
	exec { "exec_password_less_ssh" :
        ...
        ->
        check-galera-prime {"check_galera_prime":
                      openstack_index => $openstack_index,
                      os_master_ip => $os_master,
                      os_master_user => $os_username,
                      os_master_passwd => $os_passwd,
                     }
        ->
        exec { "exec_vnc_galera" :
        ...
        ->   
        check-galera {"check_galera":
                      openstack_index => $openstack_index,
                      openstack_mgmt_ip_list_shell => $openstack_mgmt_ip_list_shell,
                      openstack_user_list_shell => $openstack_user_list_shell,
                      openstack_passwd_list_shell => $openstack_passwd_list_shell,
                     }
        ->
        fix-wsrep {"fix_wsrep":
                      openstack_index => $openstack_index,
                      openstack_mgmt_ip_list_shell => $openstack_mgmt_ip_list_shell,
                      openstack_ip_list_wsrep => $openstack_ip_list_wsrep,
                  }    
