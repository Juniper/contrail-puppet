      ...
      if($internal_vip != "") {
	    ...
	    exec { "exec_set_mysql":
		    command => "openstack-config --set /etc/keystone/keystone.conf DATABASE connection mysql://keystone:$password@$host_control_ip/keystone  && 
                           openstack-config --set /etc/keystone/keystone.conf mysql connection mysql://keystone:$password@$host_control_ip/keystone && 
                           openstack-config --set /etc/cinder/cinder.conf DATABASE connection mysql://cinder:$password@$host_control_ip/cinder && 
                           openstack-config --set /etc/glance/glance-registry.conf mysql connection mysql://glance:$password@$host_control_ip/glance && 
                           openstack-config --set /etc/glance/glance-api.conf mysql connection mysql://glance:$password@$host_control_ip/glance && 
                           openstack-config --set /etc/neutron/neutron.conf mysql connection mysql://neutron:$password@$host_control_ip/neutron && 
