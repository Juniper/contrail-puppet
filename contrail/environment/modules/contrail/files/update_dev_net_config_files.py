...
#    temp_intf_file = '%s/interfaces' %(temp_dir_name)
    ...
    if not host_non_mgmt_ip:
        ...
        if 'bond' in dev.lower():
            commands.getstatusoutput("sudo sed -i '/^iface %s inet static/{n;N;d}' %s" %(dev,temp_intf_file))
        if 'bond' in dev.lower() or manual_status == 0:
            commands.getstatusoutput("sudo ifdown %s" %(dev))
