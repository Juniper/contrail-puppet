...
class GaleraSetup(ContrailSetup):
    ...
    def exec_command(cmd, ignore_failure = False, show_output = True, capture_output = False):
        ...
        try:
            if show_output:
                ...
                ret = subprocess.Popen(cmd, stdout = FNULL, shell = True)
    ...
    def fixup_config_files(self):
        ...
        self.exec_command("service contrail-hamon stop", ignore_failure = True)
        self.exec_command("service cmon stop", ignore_failure = True)
        self.exec_command("service mysql stop", ignore_failure = True)
        self.exec_command("rm -rf /var/lib/mysql/grastate.dat", ignore_failure = True)
        self.exec_command("rm -rf /var/lib/mysql/galera.cache", ignore_failure = True)
        ...
        self.exec_command("sudo mv %s/galera_param /etc/contrail/ha/" % (self._temp_dir_name))
        ...
        self.exec_command("sudo mv %s/cmon_param /etc/contrail/ha/" % (self._temp_dir_name))

        self.exec_command("echo %s > /etc/contrail/galeraid" % self._args.openstack_index)
        if self.pdist in ['Ubuntu']:
            self.exec_command("ln -sf /bin/true /sbin/chkconfig")
 ...
 #       else:
        ...
        self.exec_command("sudo mv %s/%s %s" % (self._temp_dir_name, wsrep_conf_file,
                                    wsrep_conf))
#        if self._args.openstack_index == 1:
#            local('sed -ibak "s#wsrep_cluster_address=.*#wsrep_cluster_address=gcomm://#g" %s' % (wsrep_conf))
        ...
        self.exec_command("sudo mv %s/cmon.cnf /etc/cmon.cnf" % (self._temp_dir_name))

    def install_mysql_db(self):
        self.exec_command('chkconfig %s on' % self.mysql_svc)
        self.exec_command('chown -R mysql:mysql /var/lib/mysql/')
        install_db = exec_command("service %s restart" % self.mysql_svc, ignore_failure = True)[1]
        if install_db:
            exec_command('mysql_install_db --user=mysql --ldata=/var/lib/mysql')
            self.cleanup_redo_log()
            exec_command("service %s restart" % self.mysql_svc)
    ...
    def create_mysql_token_file(self):
        ...
        if os.path.isfile(self.mysql_token_file):
            self.mysql_token = self.exec_command('cat %s' % self.mysql_token_file, capture_output = True)[0].strip()
        else:
            if os.environ.get('MYSQL_ROOT_PW'):
                self.mysql_token = os.environ.get('MYSQL_ROOT_PW')
            else:
                 self.mysql_token = self.exec_command('openssl rand -hex 10', capture_output=True)[0].strip()
            self.exec_command("echo %s > %s" % (self.mysql_token, self.mysql_token_file))
            self.exec_command("chmod 400 %s" % self.mysql_token_file)

    def get_mysql_token_file(self):
        retries = 10
        if not os.path.isfile(self.mysql_token_file):
            while retries:
                if self.exec_command('scp %s@%s:%s /etc/contrail/' % (self._args.openstack0_user, 
                                   self._args.galera_ip_list[0]), ignore_failure = True)[1] == False: 
                    time.sleep(1)
                    retries -= 1
    ...
    def set_mysql_root_password(self):
        ...
        if self.exec_command('echo show databases |mysql -u root > /dev/null', ignore_failure=True)[1] == True:
            self.exec_command('%s "use mysql; update user set password=password(\'%s\') where user=\'root\'"' 
                                        % (mysql_priv_access, self.mysql_token), ignore_failure = True)
            self.exec_command('mysqladmin password %s' % self.mysql_token, ignore_failure = True)
        elif self.exec_command('echo show databases |mysql -u root -p%s> /dev/null' % self.mysql_token, ignore_failure=True)[0]:
    ...
    def setup_grants(self):
        ...
        for host in host_list:
            self.exec_command('mysql -u root -p%s -e "CREATE USER \'root\'@\'%s\' IDENTIFIED BY %s"' % 
                               (self.mysql_token, host, self.mysql_token), ignore_failure=True, show_output=False)
            self.exec_command('%s "SET WSREP_ON=0;SET SQL_LOG_BIN=0; GRANT ALL ON *.* TO root@%s IDENTIFIED BY \'%s\'"' %
                               (mysql_cmd, host, self.mysql_token))
        self.exec_command('%s "SET wsrep_on=OFF; DELETE FROM mysql.user WHERE user=\'\'"' % mysql_cmd)
        self.exec_command('%s "FLUSH PRIVILEGES"' % mysql_cmd)

    def setup_cron(self):
        self.exec_command('crontab -l > %s/galera_cron' % self._temp_dir_name, ignore_failure = True, show_output = False)
        self.exec_command('echo "0 * * * * /opt/contrail/bin/contrail-token-clean.sh" >> %s/galera_cron' % self._temp_dir_name)
        self.exec_command('crontab %s/galera_cron' % self._temp_dir_name)
        self.exec_command('rm %s/galera_cron' % self._temp_dir_name)
    ...
    def run_services(self):
        ...
        self.cleanup_redo_log()
        if self._args.openstack_index == 1:
            self.exec_command("service %s stop" % self.mysql_svc)
            self.exec_command("service %s start --wsrep_cluster_address=gcomm://" % self.mysql_svc)
        else:
            ...
            for i in range(10):
                wsrep_local_state = self.exec_command(cmd, capture_output=True)[0].strip()
            ...
            self.exec_command("service %s restart" % self.mysql_svc)
        self.exec_command("sudo update-rc.d -f mysql remove")
        self.exec_command("sudo update-rc.d mysql defaults")
    ...
    def cleanup_redo_log(self):
        ...
        siz = self.exec_command("ls -l /var/lib/mysql/ib_logfile1 | awk '{print $5}'", ignore_failure = True, capture_output=True)[0].strip()
        if siz == self.mysql_redo_log_sz:
            self.exec_command("rm -rf /var/lib/mysql/ib_logfile*")
