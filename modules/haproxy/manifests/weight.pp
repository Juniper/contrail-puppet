# == Define Resource Type: haproxy::weight
#
...
#    same port is used for all weights.
#
# [*server_names*]
...
#    in which case a weight is created for each pair of
...
#
# [*ensure*]
#   If the weight should be present or absent.
#    Defaults to present.
#
# [*options*]
...
define haproxy::weight (
  ...
  concat::fragment { "${listening_service}_weight_${name}":
    order   => "20-${listening_service}-01-${name}",
    ensure  => $ensure,
    target  => '/etc/haproxy/haproxy.cfg',
    content => template('haproxy/haproxy_weight.erb'),
  }
}
